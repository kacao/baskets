...
import { db } from './db';
import { project, task, template } from './db/schema';
...
export async function instantiateTemplate(
	...
	await writeTaskCustomValues(parentId, projectId, fieldEntries(payload.task.fields));
	...
	for (let i = 0; i < subtasks.length; i++) {
		...
		await writeTaskCustomValues(subId, projectId, fieldEntries(sub.fields));
	}
