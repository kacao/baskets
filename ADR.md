...
- **Collapsible multi-value fields (ADR-045)**: multi-value field pills collapse by default to keep the pane compact; state persists in localStorage **per container** (task id / project id). The **Sub-tasks** (`__subtasks`) and **Description** (`__description`) sections reuse the same mechanism — Sub-tasks now defaults **collapsed** with auto-expand-on-first-add and Description defaults **expanded** (ADR-056). Pure client UI state.
...
## Project page & editing UX (ADR-012/020/023/024/025/027/030/041/044/054)
...
- **One shared SidePane** for the task pane, Customize, and Milestones; `use:portal` makes it an **in-flow flex sibling of `.content`** so opening it scrolls content rather than reflowing children. Resizable, persisted width; full-width overlay under 900px (a full-screen sheet above the topbar on phones, ADR-053). **Exactly one pane open at a time** (registry). **URL-addressable (ADR-054)**: the open pane is reflected in the URL (`?task=` / `?pane=…`) via shallow routing and restored on load.
...
## Cross-cutting (ADR-007/008/016/018/022/027/028/034/038/040/053)
...
- **Touch / mobile (ADR-042/053)**: native HTML5 DnD + `oncontextmenu` are mouse-only, so two in-house **pointer-event** primitives replace them — `use:sortable` (touch engages after a still hold so swipes still scroll) + `use:longpress` (fires the same handler as `oncontextmenu`). A global touch→DnD polyfill was rejected (would hijack Leaflet/Flow/scroll). Don't reintroduce `draggable` for new reorder UI. **Mobile pass (ADR-053)**: `viewport-fit=cover` + `env(safe-area-inset-*)` on fixed/edge UI, 16px inputs to kill iOS zoom-on-focus, full-screen task sheet above the topbar, and the "…" menu's hover flyouts become click-toggled accordions on touch.
...
## Icon library: Heroicons (ADR-052)

**Context.** The UI ran on **iconoir** (a 1383-icon sprite built from the `iconoir` npm package; `<Icon name="nav-arrow-down">` → `<use href="/iconoir.svg#…">`; the `IconPicker` searched all 1383 for user-selectable entity icons stored as `iconoir:<name>`). Requirement: switch **all** icons to **Heroicons** (Tailwind Labs, MIT). Heroicons' 24px OUTLINE set (324 icons, stroke-1.5 on currentColor) visually matches iconoir's stroke style, but uses different names and is a smaller pool — and ~60 iconoir tokens are hardcoded across 168 `<Icon>` call sites + icon maps + status/entity defaults, plus existing stored `iconoir:<name>` values.

**Decision.** Full switch to Heroicons with **zero call-site churn** via a build-time alias layer (`scripts/build-heroicons-sprite.mjs`):
- Emit `static/heroicons.svg` = one `<symbol id="<heroicon-name>">` per outline icon, PLUS **alias symbols** duplicating the mapped Heroicon under each legacy iconoir token id (curated `ALIASES` map — `nav-arrow-down`→`chevron-down`, `task-list`→`queue-list`, `edit-pencil`→`pencil-square`, `more-horiz`→`ellipsis-horizontal`, `xmark`→`x-mark`, `view-grid`→`squares-2x2`, `priority-high`→`signal`, …). So every hardcoded `<Icon name="…">` keeps working, rendering a Heroicon; `Icon.svelte` only changed its sprite URL.
- Two iconoir glyphs with no faithful Heroicon (`circle`, `git-commit`) are **hand-authored custom symbols** in the sprite.
- A build-time assertion fails if any alias target is absent from Heroicons.
- Generate `src/lib/heroiconNames.ts` with `HEROICON_NAMES` (the ~324 pickable names → `IconPicker` pool) and `ICON_NAMES` (all 367 sprite ids incl. aliases → the validation set for `parseIconValue` and the project-icon endpoints, so stored/legacy `iconoir:<token>` values still pass).
- The stored **`iconoir:` prefix is kept** as the sprite-icon namespace (a picked Heroicon stores `iconoir:<heroicon-name>`) — avoids a data migration; `EntityIcon` strips the prefix and renders via the sprite unchanged.
- Dropped the `iconoir` dependency + `scripts/build-iconoir-sprite.mjs` + `src/lib/iconoirNames.ts` + `static/iconoir.svg`; added `heroicons`.

**Consequences.** All chrome renders Heroicons (verified in-app: sidebar/topbar/table/nav — no blanks). A build assertion + a codebase token scan confirmed every referenced icon resolves. The user-selectable pool shrinks 1383→324; existing entity icons whose iconoir name isn't a curated alias fall back to nothing (accepted — common ones incl. the built-in status icons are aliased). **Rule:** new UI icons use Heroicon names directly; if a legacy iconoir token must be reused, add it to `ALIASES` (or a custom symbol) — never reintroduce the iconoir package.

## Mobile UI pass (ADR-053)

**Context.** `mobile.css` already retrofitted responsiveness (ADR-042 touch primitives + the drawer/overlay tiers) via global rules keyed on **literal class names** inside `@media` queries. A device-viewport audit (driven at 320–430px) found systemic gaps that static checks can't catch: `src/app.html` had no `viewport-fit=cover`, so **`env(safe-area-inset-*)` was inert** on notched phones (the one place `mobile.css` already used it did nothing); DaisyUI controls are <16px, so **iOS zoomed on focus**; the full-screen task sheet sat at `z-index:8` **behind** the sticky topbar (`z-index:10`), hiding its own close (×); and the project "…" menu's flyouts were **`:hover`-only**, unreachable on touch.

**Decision.** A layered, additive pass (desktop untouched — every rule lives in a `mobile.css` `@media` tier or a scoped block):
- **Safe-area foundation**: `viewport-fit=cover` in `app.html` makes the insets real; topbar padding, the drawer/backdrop `top`, the bottom bars (`.toaster`/`.bulk-bar`) and the task-sheet body/footer are wired to `env(safe-area-inset-*)`. The drawer offset mirrors the topbar's own `max(--sp-2, env-top)` formula rather than adding the inset twice.
- **iOS input/tap guards**: `16px` on text inputs/`select`/`textarea` under `@media (hover:none)` (stops zoom-on-focus); `-webkit-text-size-adjust:100%` + transparent tap-highlight on `body`.
- **True full-screen sheet**: the SidePane's own `≤900px` overlay rule now sets `z-index:40` (> topbar) so the sheet covers the topbar and its header/× are reachable (fixing a scoped-`z-index:8`-out-specifies-`mobile.css` bug).
- **Hover→click flyouts (ADR-042 follow-through)**: the "…" menu submenus are **click-toggled inline accordions** on mobile (`openSub` state + `.menu-sub.open`), with `:hover`/`:focus-within` gated behind `@media (hover:hover)` so touch has a single, non-fighting driver.
- **Per-view density**: board horizontal-scroll columns, calendar/dashboard/timeline sizing, map/flow mobile heights + larger Leaflet/Flow controls (MiniMap hidden), and the mention menu clamps to `visualViewport.height` (above the on-screen keyboard).

**Consequences.** Notched phones render edge-to-edge without content under the notch/home-indicator; forms don't zoom; the core edit loop is a proper sheet. **Rule:** any new fixed/screen-edge UI wires `env(safe-area-inset-*)`; any new hover-revealed affordance on a tap target needs the `@media (hover:none)` neutralization (the iOS "2-tap" rule). Verified at 390×844 (Playwright): topbar fit, flyout accordion, full-screen sheet ×, drawer.

## URL-addressable side panes (ADR-054)

**Context.** The SidePanes (task pane in the 6 views; Customize / Milestones / New-task / New-milestone on the project page) were **local component state**. Only the task pane read `?task=` on load (deep-link *in*); opening a pane never *wrote* the URL, so a pane wasn't copy-shareable, and the project panes had no URL at all. Ask: a pane should be reflected in the URL and restorable from it in another window.

**Decision.** Reflect the open pane in the URL via **SvelteKit shallow routing** (`replaceState`, so `load()` does NOT re-run — no refetch/flash), through one helper `src/lib/paneUrl.ts` `setPaneUrl(changes)`. The **6 views** own `?task=` — the existing read-effect (URL→state) is paired with a new **write-effect** (`selectedId`→URL); FlowView gained the deep-link it never had. The **project page** owns `?pane=customize|milestones|new-task|new-milestone` (a `?pane` read/write-effect pair + init-from-URL); the two Customize `goto('?view=…')` calls carry `&pane=customize`. The single-open-pane **registry** keeps `?task=` and `?pane=` mutually exclusive for free — opening one calls the other's `onClose`, which clears its param.

Two correctness bugs surfaced only by driving a real browser (invisible to typecheck/tests/diff):
1. **Effect echo.** The read (URL→state) and write (state→URL) effects shared a `lastParam` sentinel; because Svelte 5 `$effect` **tracks every reactive read**, changing `lastParam` in one re-ran the *other* with a stale `page.url` and clobbered the state. Fix: read the sentinel via **`untrack(() => lastParam)`** so each effect fires only on its true trigger (`page.url` for read, state for write).
2. **Stale base.** SvelteKit's shallow `replaceState`/`pushState` update the address bar but **do NOT refresh `$app/state`'s `page.url`**. Building the next URL from `new URL(page.url)` read a stale base, so a close-after-open deduped against the wrong URL and no-op'd. Fix: base `setPaneUrl` on **`window.location.href`** (bonus: our own writes then never echo into the read-effect — only real `popstate` does).

**Consequences.** Every pane is linkable/restorable (verified: open→URL, close→clear, deep-link→restore, task↔pane mutual exclusion, browser-Back consistent, across all view types). **`replaceState` (not `pushState`)** was chosen so in-pane task→task nav doesn't fill history; browser Back leaves the page (implicitly closing the pane) rather than stepping panes. **Rule:** reflect new panes through `setPaneUrl` (never hand-roll `history`/`goto` for a pane), and read any URL↔state sentinel via `untrack`. **Deferred:** `pushState`-on-open (Back-closes-pane) — a UX nicety, not required.

## Side-pane event & effect robustness (ADR-055)

**Context.** Driving the task pane in a real browser surfaced a family of bugs (invisible to typecheck/tests/diff, same discovery mode as ADR-054) where an event or effect the pane doesn't own closed or clobbered it: (a) clicking any pane input then clicking outside closed the pane; (b) a native `<input type=date>` calendar's Escape closed the pane; (c) an open confirm dialog's Escape closed the pane behind it (and its Enter could confirm a delete from a background field); (d) an in-progress custom-field edit was reset by an unrelated save/realtime ping; (e) the portaled project header flashed in `.content` before hydration moved it.

**Decision.**
- **Read the LIVE url, not stale `page.url` (read-side complement to ADR-054).** ADR-054 fixed the WRITE side (base `setPaneUrl` on `window.location`); the READ side had the same flaw. Each view's read-effect (URL→`selectedId`) tracked `page.url` but read its VALUE — which shallow routing leaves stale (missing `?task=`). Any `invalidateAll()` (an input's blur auto-save; a realtime ping) churns `page` and re-runs the effect, which read the stale URL and cleared `selectedId` → pane closed. New helper **`readPaneParam(key)`** in `paneUrl.ts` *touches* `page.url` (so real back/forward still re-trigger) but *returns* the value from `window.location`. Applied to all 6 views' `?task=` effect + the project page's `?pane=` effect.
- **Escape hygiene = stop it in the CAPTURE phase.** Two `svelte:window` Escape listeners on the same target aren't reliably ordered, so bubble-phase `stopPropagation` can't guarantee an inner handler pre-empts an outer one. `Popover` and `ConfirmModal` register **capture-phase** window keydown listeners that `stopPropagation()`, so Escape dismisses the popover/dialog and never reaches `SidePane`'s Escape-closes-pane handler. `ConfirmModal` also scopes **Enter**-confirm to focus inside the dialog (an Enter typed in a background field can't confirm a danger delete). `SidePane` additionally ignores Escape whose target is a native date/time input, and any Escape while a confirm dialog is open (belt-and-suspenders).
- **Guard effects that seed state from churned data.** `CustomFieldValue`'s editor binds `value={current}` one-way, so keystrokes live in the DOM until blur; an unguarded `$effect(() => current = value)` reset the input on any `invalidateAll`. It now reseeds only when the field/task identity switches OR the server value genuinely changed (a `lastServerValue` + key sentinel), never on same-value churn.
- **Hide-until-portaled (complements ADR-044).** The shared `portal` action stamps `data-portaled`; the project header is `display:none` until then, so it never paints in `.content` (its SSR position) and teleports on hydration.

**Consequences.** The pane survives input-blur auto-saves, native-picker Escapes, confirm-cancel, and realtime pings; no header flash. **Rule:** a URL↔pane read-effect reads via `readPaneParam` (never `page.url.searchParams` for a shallow param); a modal/popover that can open over the pane stops Escape in the capture phase; an effect seeding local state from `page.data`-derived props guards on identity/value. Locked by `tests/unit/paneUrl.test.ts` + `tests/e2e/pane-behavior.spec.ts`.

## Task-pane input UX (ADR-056)

**Context.** Several pane inputs had friction or off defaults: the date pills opened a Popover holding a native `<input type=date>` (a two-step mm/dd/yyyy field, and the source of the native-Escape pane-close in ADR-055); number custom fields were a bare `<input type=number>` with ugly native spinners and no unit context; the Sub-tasks section defaulted expanded (noisy) with its "+ Add" buried in the body; Description was always-on with no collapse.

**Decision.**
- **Custom calendar `DatePicker.svelte`** — a fully in-DOM month grid (prev/next · Today · Clear), controlled (`value`/`onSelect`/`onClear`). Replaces the native date input in every task date **pill** (Start/Due, sub-task Due, task-cf Due): one click opens the calendar, no native picker (which also removes the native-Escape leak). Parses `yyyy-mm-dd` as a LOCAL date (avoids the UTC-shift of `new Date('yyyy-mm-dd')`). `CustomFieldValue`'s `date`/`datetime-local` editor stays native (time support) — protected by the ADR-055 Popover Escape fix.
- **Number editor** — custom −/+ steppers (touch-friendly; accumulate locally, one save on blur — no per-click `invalidateAll`), native spinners hidden, right-aligned tabular figures, and a currency/unit **affix** (`$`, `€`…) from `numberAffix(config)`.
- **Sub-tasks section defaults COLLAPSED** (reverses ADR-045's expanded default) and **auto-expands + persists the first time a sub-task is added** (0→≥1). The add affordance is an **icon-only "+" pinned to the section header** (works while collapsed).
- **Description is a collapsible section** (default expanded, persisted per task under `__description` via the ADR-045 `cfCollapse` mechanism) with a **borderless, document-like editor** (no border/shadow/ring; faint hover/focus bg — mirrors the Overview page).

**Consequences.** Dates are one-click; numbers are legible + touch-friendly; the pane opens compact (subs collapsed) yet reveals subs as you create them; Description can be tucked away. `numberAffix` is unit-tested; the behaviors are covered by `tests/e2e/pane-behavior.spec.ts`.
