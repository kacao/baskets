...
- Statuses: table-driven (`status` + `project_status` eligibility). Behavior keys off `status.category` (`todo|active|done|canceled`), never the name. Built-ins bootstrapped in `src/lib/server/statuses.ts` (`ensureDefaultStatuses`, called from hooks) — keep `scripts/seed.ts` in sync. Admin CRUD at /settings/statuses; labels (+groups) at /settings/labels.
- Views: `view` table, one Svelte component per type in `src/lib/components/views/` (table/board/dashboard/map). Project must keep ≥1 view; default Table view created by `createProjectWithDefaults` (`src/lib/server/projects.ts`) — always create projects through that helper. View config is schemaless JSON; treat keys as optional hints. Map uses Leaflet (client-only import), `task.location` = "lat, lng".
- Permissions: reads open to all signed-in users; edits need admin or a grant row (`permission` table). Check via `src/lib/server/permissions.ts` (`canEditProject/View/Task`) in EVERY mutation (form action + REST). Project grant cascades to its views/tasks; parent-task grant covers sub-tasks. Grants managed by admins in the project Edit panel.
