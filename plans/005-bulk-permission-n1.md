...
row. Per ADR-019 (inlined below) task-edit permission *is* project access —
...
`await` never fires (no queries). The N+1 only bites a user who is *not* a
...
- The behavior contract: a task in a *different* project must still be excluded.
				...
				await db
					...
					.where(
						and(
							...
							inArray(permission.resourceId, visibleViews.map((v) => v.id))
						)
					)
			).map((r) => r.id)
		);
